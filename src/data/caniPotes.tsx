import { Project } from './../interfaces/Project';
import img from './caniPotes.jpg';

export const canipotes: Project = {
  name: "Cani' Potes",
  year: 2021,
  coverImg: img,
  coverImgAlt: "Cani' Potes project",
  stack: ['React', 'Redux', 'SASS', 'NodeJs', 'Leaflet'],
  ghLink: 'https://github.com/RaphaelCharousset/cani-potes',
  urlLink: 'http://cani-potes.surge.sh/',
  description: "Projet de fin d'études, réalisé en 1 mois en équipe de 5 développeurs. Site de rencontre pour les propriétaires de chiens via une carte interactive."
}